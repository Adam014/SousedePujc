...
import { format, isBefore, isSameDay, addMonths, subMonths, startOfMonth, endOfMonth } from "date-fns"
import { cs } from "date-fns/locale"
import { Button } from "@/components/ui/button"
import { ChevronLeft, ChevronRight } from "lucide-react"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { RENTAL_DISCOUNTS } from "@/lib/constants"
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover"
import { Badge } from "@/components/ui/badge"
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip"

import { getDaysInMonth, getMonths, getYears, calculatePricing, type CalendarDay } from "./booking-calendar.utils"
import { useBookedDates } from "./use-booked-dates"
...
export default function BookingCalendar({
  ...
  const { bookedDates, loading, error } = useBookedDates(itemId)
  const {
    isDateBooked,
    isDatePending,
    isDateDisabled,
    ...
    handleDateClick,
    handleQuickSelect,
  } = useDateSelection({ bookedDates, selectedDates, onSelect })
  ...
  }, [currentMonth, currentYear, bookedDates, isDateDisabled])

  // Validate and adjust initial "from" date if it's unavailable
  ...
  const handleMonthChange = (month: string) => setCurrentDate(new Date(currentYear, Number.parseInt(month), 1))
  const handleYearChange = (year: string) => setCurrentDate(new Date(Number.parseInt(year), currentMonth, 1))

  const pricing = calculatePricing(selectedDates, dailyRate)
